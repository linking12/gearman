...
public class MetricsEngine implements QueueMetrics {
    ...
    public MetricsEngine(MetricRegistry registry){
        ...
        pendingJobsGauge = registry.register(name("queues", "pending-jobs"),
                                             (Gauge<Long>) MetricsEngine.this::getPendingJobsCount);
    ...
    public void handleWorkerRemoval(EngineWorker worker) {
        ...
        for (String jobQueue : worker.getAbilities()) {
            if (workerCounters.get(jobQueue) != null) workerCounters.get(jobQueue).dec();
        }
    }
    ...
    public long getPendingJobsCount() {
        return queueCounters.values().stream().mapToLong(counterGroup -> counterGroup.total.getValue()).sum();
    }

    @Override
    public long getPendingJobsCount(String queueName) {
        return queueCounters.get(queueName).low.getValue() + queueCounters.get(queueName).mid.getValue()
