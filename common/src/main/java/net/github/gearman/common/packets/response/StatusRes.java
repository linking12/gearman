...
public class StatusRes extends ResponsePacket {
    ...
    public byte[] toByteArray() {
        ...
        byte[] metadata = stringsToTerminatedByteArray(false, // No final terminator
                                                       jobHandle.get(), String.valueOf(knownStatus),
    ...
    public int getPayloadSize() {
        return jobHandle.get().length() + 1 + 2 + 2 + String.valueOf(numerator).length() + 1
               + String.valueOf(denominator).length();
    }
    ...
    public JobStatus toJobStatus() {
        return new JobStatus(numerator, denominator, this.running ? JobState.WORKING : JobState.QUEUED, getJobHandle());
    }
}
