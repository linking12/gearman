...
public class JobAssignUniq extends ResponsePacket {
    ...
    public int getPayloadSize() {
        return this.jobHandle.get().length() + 1 + this.functionName.get().length() + 1 + this.uniqueId.get().length()
               + 1 + this.data.length;
    }
    ...
    public Job getJob() {
        return new Job.Builder().jobHandle(this.jobHandle.get()).data(this.data).functionName(this.functionName.get()).uniqueID(this.uniqueId.get()).build();
    }
}
