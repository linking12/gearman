...
public class JobQueueMetrics {
    ...
    public final TimeSeries queued, failed, completed, exceptions, highJobs, midJobs, lowJobs, futureJobs;
    ...
    public JobQueueMetrics(TimeSeries queued, TimeSeries failed, TimeSeries completed, TimeSeries exceptions,
                           TimeSeries highJobs, TimeSeries midJobs, TimeSeries lowJobs, TimeSeries futureJobs){
        this.queued = queued;
        this.failed = failed;
    ...
    public JobQueueMetrics compact() {
        return new JobQueueMetrics(compactTimeSeries(queued, SampleMethod.MEAN),
    ...
    private TimeSeries compactTimeSeries(TimeSeries original, SampleMethod method) {
        TimeSeries ts = original.downSample(Frequency.of(5, TimeUnit.MINUTES), method);
