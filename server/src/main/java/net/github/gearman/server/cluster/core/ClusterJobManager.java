...
public class ClusterJobManager extends JobManager implements MessageListener<WorkMessage> {
    ...
    public ClusterJobManager(JobQueueFactory jobQueueFactory, JobHandleFactory jobHandleFactory,
                             UniqueIdFactory uniqueIdFactory, HazelcastInstance hazelcast, QueueMetrics queueMetrics){
    ...
    public synchronized void handleWorkCompletion(Job job, byte[] data) {
        ...
        WorkMessage workCompleteMessage = new WorkMessage(job.getUniqueID(), job.getFunctionName(),
                                                          PacketType.WORK_COMPLETE, data);
    ...
    public synchronized Job storeJobForClient(Job job, EngineClient client) {
        try {
            ...
            WorkMessage newJobMessage = new WorkMessage(storedJob.getUniqueID(), storedJob.getFunctionName(),
                                                        PacketType.SUBMIT_JOB, new byte[0]);
    ...
    public void onMessage(Message<WorkMessage> message) {
        ...
        switch (workMessage.type) {
            ...
            case SUBMIT_JOB:
                ...
                getWorkerPool(workMessage.functionName).wakeupWorkers();
                break;
            case WORK_DATA:
                ...
                if (dataJob != null) {
                    LOG.debug("Notifying any pending clients of updated data  " + workMessage.uniqueId
                              + " is complete.");
